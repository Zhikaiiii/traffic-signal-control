...
def output_road_types():
    str_types = '<types>\n'
    str_types += '  <type id="a" priority="2" numLanes="3" speed="%.2f"/>\n' % SPEED_LIMIT_ST
    str_types += '  <type id="b" priority="1" numLanes="3" speed="%.2f"/>\n' % SPEED_LIMIT_AV
    str_types += '</types>\n'
    return str_types
...
def output_netconfig():
    ...
    str_config += '    <edge-files value="Grid25.edg.xml"/>\n'
    str_config += '    <node-files value="Grid25.nod.xml"/>\n'
    str_config += '    <type-files value="Grid25.typ.xml"/>\n'
    str_config += '    <tllogic-files value="Grid25.tll.xml"/>\n'
    str_config += '    <connection-files value="Grid25.con.xml"/>\n'
    str_config += '  </input>\n  <output>\n'
    str_config += '    <output-file value="Grid25.net.xml"/>\n'
    str_config += '  </output>\n</configuration>\n'
    return str_config
...
def init_routes(density):
    ...
    for i in range(1, 25, 5):
        for j in range(4):
            ...
            k += 1
            output += get_od(node1, node2, k, lane=2)
            k += 1
            output += get_od(node2, node1, k, lane=2)
            k += 1
    # avenues
    for i in range(1, 6):
        for j in range(0, 20, 5):
...
def output_flows(peak_flow1, peak_flow2, density, seed=None):
    ...
    ratios1 = np.array([0.1, 0.3, 0.5, 0.7, 0.9, 1.0, 0.85, 0.65, 0.45, 0.3, 0.15, 0.1])
    ratios2 = np.array([1.0, 0.8, 0.6, 0.4, 0.2, 0.1, 0.25, 0.45, 0.65, 0.8, 0.95, 1.0])
...
def gen_rou_file(path, peak_flow1, peak_flow2, density, seed=None, thread=None):
    if thread is None:
        flow_file = 'Grid25.rou.xml'
    else:
        flow_file = 'Grid25_%d.rou.xml' % int(thread)
    write_file(path + flow_file, output_flows(peak_flow1, peak_flow2, density, seed=seed))
    sumocfg_file = path + ('Grid25_%d.sumocfg' % thread)
...
def output_config(thread=None):
    if thread is None:
        out_file = 'Grid25.rou.xml'
    else:
        out_file = 'Grid25_%d.rou.xml' % int(thread)
    str_config = '<configuration>\n  <input>\n'
    str_config += '    <net-file value="Grid25.net.xml"/>\n'
    str_config += '    <route-files value="%s"/>\n' % out_file
    str_config += '    <additional-files value="Grid25.add.xml"/>\n'
    str_config += '  </input>\n  <time>\n'
...
def output_tls(tls, phase):
    str_adds = '<additional>\n'
    # all crosses have 4 phases
    phases = ['GGGrrrrrGGGrrrrr', 'yyyrrrrryyyrrrrr',
              'rrrGrrrrrrrGrrrr', 'rrryrrrrrrryrrrr',
              'rrrrGGGrrrrrGGGr', 'rrrryyyrrrrryyyr',
...
def main():
    ...
    write_file('./Grid25.nod.xml', output_nodes(node))

    # typ.xml file
    write_file('./Grid25.typ.xml', output_road_types())

    # edg.xml file
    edge = '  <edge id="%s" from="%s" to="%s" type="%s"/>\n'
    write_file('./Grid25.edg.xml', output_edges(edge))

    # con.xml file
    con = '  <connection from="%s" to="%s" fromLane="%d" toLane="%d"/>\n'
    write_file('./Grid25.con.xml', output_connections(con))

    # tls.xml file
    tls = '  <tlLogic id="%s" programID="0" offset="0" type="static">\n'
    phase = '    <phase duration="%d" state="%s"/>\n'
    write_file('./Grid25.tll.xml', output_tls(tls, phase))

    # net config file
    write_file('./Grid25.netccfg', output_netconfig())

    # generate net.xml file
    os.system('netconvert -c Grid25.netccfg')

    # raw.rou.xml file
    write_file('./Grid25.rou.xml', output_flows(1200, 1200, 0.2))

    # generate rou.xml file
    # os.system('jtrrouter -n exp.net.xml -r exp.raw.rou.xml -o exp.rou.xml')

    # add.xml file
    ild = '  <laneAreaDetector file="ild.out" freq="1" id="%s_%d" lane="%s_%d" pos="-100" endPos="-1"/>\n'
    # ild_in = '  <inductionLoop file="ild_out.out" freq="15" id="ild_in:%s" lane="%s_0" pos="10"/>\n'
    write_file('./Grid25.add.xml', output_ild(ild))

    # config file
    write_file('./Grid25.sumocfg', output_config())
