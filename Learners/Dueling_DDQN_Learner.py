...
class Dueling_DDQN_Learner(Base_Learner):
    def __init__(self, config):
        ...
        self.q_network_current = self.create_model(self.input_dim, self.output_dim)
        self.q_network_target = self.create_model(self.input_dim, self.output_dim)
        ...
        # 复制网络
        # a = self.q_network_current.parameters()
        ...
        # self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'max', patience=3, threshold=1e-6,
        #                                                       factor=0.2)
    ...
    def cal_current_actions_value(self, embedding_next, embedding_next_target, rewards, is_dones):
        # self.q_network_current.eval()
        # with torch.no_grad():
        out_next = self.q_network_current(embedding_next).detach()
        ...
        # with torch.no_grad():
        out_target = self.q_network_target(embedding_next_target)
    ...
    def cal_expected_actions_value(self, embedding, actions):
        # with torch.no_grad():
        out = self.q_network_current(embedding)
        advantages_values, state_values = out[:, :-1], out[:, -1]
